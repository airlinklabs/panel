(function () {
  ...
  function renderPortRows() {
    ...
    if (!portsState.length) {
      requiredPortsList.innerHTML =
        '<p class="text-xs text-neutral-500 dark:text-neutral-400">No ports yet. Add one below, or leave empty for a single auto-assigned port.</p>';
      return;
    }
    portsState.forEach(function (port, index) {
      ...
      removeBtn.className =
        "px-2 py-2 rounded-lg text-neutral-500 hover:text-red-500 hover:bg-red-500/10 transition";
      removeBtn.title = "Remove port";
      removeBtn.setAttribute("aria-label", "Remove port");
      removeBtn.innerHTML =
        '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3a6 6 0 0 0 9 4.5v9A6 6 0 0 1 6 21a6 6 0 0 1-3-5.2V6.5A6 6 0 0 0 12 3"/></svg>'.replace(
          /<svg/g,
          "<svg ",
        );
  ...
  document.querySelectorAll(".stepper-btn").forEach(function (btn) {
    btn.addEventListener("click", function () {
      ...
      var min =
        ...
        parseInt(input.min) ||
        STEPPER_DEFAULTS.min;
      var max =
        parseInt(btn.dataset.max) ||
        parseInt(input.max) ||
        STEPPER_DEFAULTS.max;
  ...
  function refreshHeadroom() {
    ...
    if (data.ram > 0)
      rows.push(
        headroomRow(
          ...
          data.ram *
            STORAGE_UNITS.gb *
            (1 + data.overMemory / STORAGE_UNITS.overflowPct),
          "MB",
          fmtMb,
        ),
      );
    if (data.cpu > 0)
      rows.push(
        headroomRow(
          ...
          data.usedCpu,
          cpu,
          data.cpu * (1 + data.overCpu / STORAGE_UNITS.overflowPct),
          "%",
          fmtPercent,
        ),
      );
    if (data.disk > 0)
      rows.push(
        headroomRow(
          ...
          data.disk *
            STORAGE_UNITS.gb *
            (1 + data.overDisk / STORAGE_UNITS.overflowPct),
          "MB",
          fmtMb,
        ),
      );
    ...
    rows.forEach(function (r) {
      nodeHeadroomRows.appendChild(r);
    });
    var over = data.overMemory > 0 || data.overCpu > 0 || data.overDisk > 0;
    headroomHint.textContent = over
      ? "Node capacity includes overallocation."
      : "";
  ...
  document.addEventListener("click", function (e) {
    if (e.target && e.target.closest && e.target.closest(".al-format-switcher"))
      refreshHeadroom();
  });
  seedPortsFromImage();
  refreshHeadroom();
  ...
  createBtn.addEventListener("click", async function () {
    ...
    if (mem < MIN_MEMORY)
      validationErrors.push("Memory must be at least " + MIN_MEMORY + " MB.");
    if (storage < MIN_STORAGE)
      validationErrors.push("Storage must be at least " + MIN_STORAGE + " MB.");
    if (cpu < MIN_CPU)
      validationErrors.push("CPU must be at least " + MIN_CPU + "%.");

    if (
      portsState.some(function (entry) {
        return (
          !entry.name ||
          !Number.isInteger(entry.internalPort) ||
          entry.internalPort < 1 ||
          entry.internalPort > 65535
        );
      })
    ) {
      validationErrors.push(
        "Every port needs a name and a valid internal port (1-65535).",
      );
    }
    ...
    var ok = await showConfirm(
      ...
      '"' +
        name +
        '" will be created and queued for installation. This may take a moment.',
    );
    if (!ok) {
      btn.disabled = false;
      btn.textContent = origText;
    ...
    try {
      ...
      if (d.success) {
        if (toastContainer) {
          ...
          toast.innerHTML =
            '<span class="al-toast-icon">' +
            alIcon("circle-check", "w-4 h-4", { strokeWidth: 2 }) +
            '</span><span class="al-toast-text">Server created successfully!</span>';
