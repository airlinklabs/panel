(function () {
  ...
  var CHART_TICK_COLOR =
    _rootStyle.getPropertyValue("--theme-text").trim() || "#FFFFFF";
  ...
  async function fetchPlayerData() {
    try {
      ...
      if (data.historicalData && data.historicalData.length > 0) {
        const labels = data.historicalData.map((entry) =>
          new Date(entry.timestamp).toISOString(),
        );
        const playerCounts = data.historicalData.map(
  ...
  document
    ...
    .addEventListener("click", fetchPlayerData);
  ...
  function wirePlayerStatsRealtime() {
    ...
    unsubscribeRealtime = st.observe("admin:playerstats", function () {
      fetchPlayerData();
    });
  }
