(function () {
  ...
  var CAT_COLOR = {
    game:
      _rootStyle.getPropertyValue("--theme-accent-purple").trim() || "#7c3aed",
    application:
      _rootStyle.getPropertyValue("--theme-accent-blue").trim() || "#0284c7",
  ...
  function catBadge(cat) {
    return (
      '<span class="inline-flex items-center gap-1.5 rounded-md px-2 py-0.5 text-xs font-medium ring-1 ring-inset ' +
      (CAT_CLS[cat] || "") +
      '">' +
      catDot(cat, 6) +
      (CAT_LABEL[cat] || cat) +
  ...
  function groupData() {
    ...
    allImages.forEach(function (img) {
      if (!map[img.group])
        map[img.group] = {
          ...
          readme: img.groupReadme || "",
        };
      map[img.group].imgs.push(img);
      if (img.description && !map[img.group].desc)
        map[img.group].desc = img.description;
    });
    ...
    out.sort(function (a, b) {
      var va, vb;
      if (sortBy === "type") {
        va = CAT_LABEL[a.domCat] || "";
        vb = CAT_LABEL[b.domCat] || "";
  ...
  async function loadCatalogue() {
    ...
    try {
      ...
      hide("loadingEl");
      var age = data.builtAt
        ? Math.round((Date.now() - data.builtAt) / 60000)
        : 0;
      if (st)
        st.textContent =
          allImages.length +
          " images \u00B7 " +
          (age < 1 ? "fresh" : age + "m old");
      updateCount();
      render();
    } catch (e) {
  ...
  async function doRefresh() {
    ...
    try {
      before =
        (await (await fetch("/admin/images/store/catalogue")).json()).builtAt ||
        0;
    } catch (e) {
      console.error("Failed to fetch catalogue timestamp:", e);
    }
    try {
      var refreshRes = await fetch("/admin/images/store/refresh", {
        method: "POST",
      });
      ...
      if (typeof showToast === "function")
        showToast("Could not refresh the catalogue", "error");
      return;
    }
    for (var i = 0; i < 60; i++) {
      ...
      try {
        var cur =
          (await (await fetch("/admin/images/store/catalogue")).json())
            .builtAt || 0;
        if (cur !== before) break;
      } catch (e) {
        /* polling retry */
  ...
  function render() {
    ...
    updateCount();
    if (!groups.length) {
      document.getElementById("emptyTxt").textContent = allImages.length
        ? "No apps match your search."
        : "The image catalogue is empty.";
      show("emptyEl");
      return;
    }
  ...
  function makeGroupRow(g) {
    ...
    tr.innerHTML =
      ...
      catDot(g.domCat) +
      '<span class="text-sm font-medium truncate" style="color:var(--theme-text-strong)">' +
      esc(cap(g.group)) +
      "</span>" +
      "</div>" +
      "</td>" +
      '<td class="al-table-td whitespace-nowrap px-3 py-4">' +
      catBadge(g.domCat) +
      "</td>" +
      '<td class="al-table-td whitespace-nowrap px-3 py-4 text-sm" style="color:var(--theme-text-muted)">' +
      g.imgs.length +
      " image" +
      (g.imgs.length !== 1 ? "s" : "") +
      "</td>" +
      '<td class="al-table-td px-3 py-4 text-sm" style="color:var(--theme-text-muted)">' +
      (g.readme ? esc(g.readme.replace(/[#*`\[\]]/g, "")) : esc(g.desc || "")) +
      "</td>";
    tr.addEventListener("click", function () {
      openGroup(g.group, g.imgs);
    });
    return tr;
  }
  ...
  function openGroup(group, imgs) {
    ...
    s.forEach(function (img, i) {
      ...
      var subLabel =
        img.subGroup && img.subGroup !== img.group
          ? img.subGroup.replace(img.group + "/", "")
          : "";
      row.innerHTML =
        '<div class="flex items-center gap-2 min-w-0">' +
        catDot(img.category) +
        '<div class="min-w-0">' +
        '<p class="text-xs font-medium truncate" style="color:var(--theme-text-strong)">' +
        esc(img.name) +
        "</p>" +
        (subLabel
          ? '<p class="text-[11px] font-mono truncate" style="color:var(--theme-text-muted)">' +
            esc(subLabel) +
            "</p>"
          : "") +
        "</div>" +
  ...
  async function getMd() {
    ...
    try {
      var mod = await import("/vendor/marked/lib/marked.esm.js");
      mdParse = mod.marked;
  ...
  async function renderMd(md, el) {
    el.innerHTML =
      '<p class="text-xs" style="color:var(--theme-text-muted);font-style:italic">Rendering\u2026</p>';
    try {
      var m = await getMd();
      el.innerHTML =
        '<div class="md-body">' +
        (md
          ? m.parse(md)
          : '<p style="font-style:italic;color:#a3a3a3;font-size:12px;">No readme available.</p>') +
        "</div>";
    } catch (e) {
  ...
  function openEgg(img) {
    ...
    catEl.className =
      "text-[10px] font-medium px-2 py-0.5 rounded-full inline-flex items-center gap-1.5 ring-1 ring-inset " +
      (CAT_CLS[img.category] || "");
    catEl.innerHTML =
      catDot(img.category, 6) + (CAT_LABEL[img.category] || img.category);
    document.getElementById("eggDesc").textContent = img.description || "";
    document.getElementById("eggAuthor").textContent = img.author
      ? "by " + img.author
      : "";
    renderMd(
      img.fullReadme || img.readme || "",
      document.getElementById("eggReadme"),
    );
    hide("eggErr");
  ...
  async function confirmInstall() {
    ...
    try {
      ...
      if (!res.ok) {
        document.getElementById("eggErrTxt").textContent =
          body.error || "Installation failed.";
        show("eggErr");
        btn.disabled = false;
        btn.innerHTML = "Install";
        return;
      ...
      btn.innerHTML =
        '<svg class="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg> Installed';
      btn.style.background = "var(--theme-success, #16a34a)";
      btn.style.borderColor = "var(--theme-success, #16a34a)";
      btn.style.pointerEvents = "none";
      if (typeof showToast === "function")
        showToast('"' + installedName + '" installed successfully.', "success");
      setTimeout(function () {
        closeEgg();
      }, 1500);
    } catch (err) {
  ...
  function mount(root) {
    ...
    var input = mountedRoot.querySelector
      ? mountedRoot.querySelector("#image-store-search-input")
      : null;
    ...
    if (
      mountedRoot &&
      mountedRoot.addEventListener &&
      !mountedRoot._alStoreSortBound
    ) {
      mountedRoot._alStoreSortBound = true;
      mountedRoot.addEventListener("click", function (e) {
        var btn =
          e.target && e.target.closest ? e.target.closest("[data-sort]") : null;
