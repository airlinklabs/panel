(function () {
  function handleRowClick(e, url) {
    if (!e.target.closest("button,a")) window.location = url;
  }
  ...
  function imageRowHtml(image) {
    return (
      '<tr class="al-table-tr transition-colors cursor-pointer img-row" data-search="' +
      esc((image.name + " " + (image.author || "")).toLowerCase()) +
      '" onclick="handleRowClick(event, \'/admin/images/edit/' +
      image.id +
      "')\">" +
      '<td class="al-table-td whitespace-nowrap py-4 pl-6 pr-3 font-medium" style="color:var(--theme-text-strong)">' +
      esc(image.name) +
      "</td>" +
      '<td class="al-table-td whitespace-nowrap px-3 py-4 col-hide">' +
      esc(image.author || "—") +
      "</td>" +
      '<td class="al-table-td whitespace-nowrap px-3 py-4 col-hide">' +
      esc(new Date(image.createdAt).toLocaleDateString()) +
      "</td>" +
      '<td class="whitespace-nowrap px-3 py-4 text-sm"><div class="flex gap-2">' +
      '<a href="/admin/images/edit/' +
      image.id +
      '" onclick="event.stopPropagation()" class="al-btn-secondary px-3 py-2 text-sm font-medium shadow-lg transition inline-flex items-center gap-1.5">' +
      (window.alIcon ? alIcon("pencil", "size-4") : "") +
      "Edit</a>" +
      '<a href="/admin/images/export/' +
      image.id +
      '" onclick="event.stopPropagation()" class="al-btn-secondary px-3 py-2 text-sm font-medium shadow-lg transition inline-flex items-center gap-1.5">' +
      (window.alIcon ? alIcon("download", "size-4") : "") +
      "Export</a>" +
      "<button onclick=\"event.stopPropagation(); openDelete('" +
      image.id +
      "', '" +
      esc(image.name).replace(/'/g, "\\'") +
      '\')" type="button" class="al-btn-danger" aria-label="Delete image ' +
      esc(image.name) +
      '">' +
      (window.alIcon ? alIcon("trash-2", "size-4") : "") +
      "</button>" +
      "</div></td></tr>"
    );
  }
  ...
  async function renderImageTable() {
    ...
    if (emptyState)
      emptyState.classList.toggle("hidden", data.images.length > 0);
    if (tableSection)
      tableSection.classList.toggle("hidden", data.images.length === 0);
    if (!tbody) return;
  ...
  async function deleteImage() {
    if (!_deleteId) return;
    const res = await fetch("/admin/images/delete/" + _deleteId, {
      method: "DELETE",
    });
    if (res.ok) {
  ...
  document
    .getElementById("imageFilterInput")
  ...
  function setUploadMode(mode) {
    ...
    const active =
      "color:var(--theme-text-strong); background:var(--theme-bg-card);";
  ...
  function closeUploadImageModal() {
    ...
    Animate.closeModal(
      modal,
      document.getElementById("uploadImageModalPanel"),
      function () {
  ...
  function handleImageFileSelection(file) {
    ...
    document.getElementById("imageSelectedFileSize").textContent =
  ...
  function confirmImageUpload() {
    if (!window.selectedImageFile) {
      showToast("Select a JSON file to upload.", "error");
      return;
    }
  ...
  document
    ...
    .addEventListener("click", openUploadImageModal);
  ...
  document
    .getElementById("uploadModeFile")
    .addEventListener("click", () => setUploadMode("file"));
  document.getElementById("uploadModeUrl").addEventListener("click", () => {
    setUploadMode("url");
    document.getElementById("importUrlInput").focus();
  });

  async function submitCreateImage(e) {
    const form = e.target;
    const data = Object.fromEntries(new FormData(form));
    if (!data.name || !data.startup) {
      showToast("Name and startup command are required.", "error");
      return;
    }
    const csrfToken =
      document.querySelector('meta[name="csrf-token"]')?.content || "";
    try {
      const res = await fetch("/admin/images/create", {
        method: "POST",
        headers: {
      ...
      if (res.ok && result.success !== false) {
        ...
        showToast(
          result.error || result.message || "Failed to create image.",
          "error",
        );
      }
    } catch {
  ...
  window.submitCreateImage = submitCreateImage;
})();
