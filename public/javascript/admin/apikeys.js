function showConfirmModal(title, message, onConfirm) {
  ...
  modal.innerHTML = `
    <div class="al-surface rounded-2xl p-6 max-w-sm w-full mx-4 shadow-2xl">
      <h2 class="text-sm font-semibold mb-1" style="color:var(--theme-text-strong)">${title}</h2>
      <p class="text-xs mb-5" style="color:var(--theme-text-muted)">${message}</p>
      <div class="flex gap-2 justify-end">
        <button id="modalCancel" class="al-btn-secondary">Cancel</button>
        <button id="modalConfirm" class="al-btn-danger">Delete</button>
      </div>
    </div>`;
