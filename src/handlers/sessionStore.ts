...
class RedisSessionStore extends session.Store {
  ...
  get(sid: string, callback: (err: Error | null, session?: session.SessionData) => void): void {
    ...
    this.redis.set(`${this.prefix}${sid}`, data, 'EX', ttlSec).then(() => {
      try {
        ...
        if (userId != null) {
          ...
          this.redis.pipeline().sadd(key, sid).expire(key, ttlSec).exec().catch(() => {});
        }
      } catch {
  ...
  destroy(sid: string, callback?: (err?: Error) => void): void {
    this.redis.get(`${this.prefix}${sid}`).then((data) => {
      ...
      pipeline.exec().then(() => callback?.()).catch((err) => callback?.(err as Error));
    }).catch((err) => {
      callback?.(err as Error);
    });
  }
  ...
  all(callback: (err: Error | null, sessions?: session.SessionData[]) => void): void {
    this.getAllKeys().then((keys) => {
      if (keys.length === 0) {
        callback(null, []);
        return;
      }
      this.redis.mget(...keys).then((results) => {
  ...
  clear(callback?: (err?: Error) => void): void {
    this.getAllKeys().then((keys) => {
      ...
      this.redis.del(...keys).then(() => callback?.()).catch((err) => callback?.(err as Error));
    }).catch((err) => callback?.(err as Error));
  }
  ...
  private async getAllKeys(): Promise<string[]> {
    ...
    let cursor = '0';
    do {
      const [nextCursor, batch] = await this.redis.scan(cursor, 'MATCH', `${this.prefix}*`, 'COUNT', 200);
      cursor = nextCursor;
      keys.push(...batch);
    } while (cursor !== '0');
    return keys;
