...
router.post('/', parseBody(adminCreateServerBody), async (req, res) => {
  ...
  if (!owner) {return jsonError(res, 'NOT_FOUND', 'Owner not found', 404);}

  // Verify node exists
  const node = await prisma.node.findUnique({ where: { id: data.nodeId } });
  if (!node) {return jsonError(res, 'NOT_FOUND', 'Node not found', 404);}

  // Verify image exists
  const image = await prisma.images.findUnique({ where: { id: data.imageId } });
  if (!image) {return jsonError(res, 'NOT_FOUND', 'Image not found', 404);}
...
router.get('/:id', async (req, res) => {
  ...
  if (isNaN(id)) {return jsonError(res, 'BAD_REQUEST', 'Invalid server ID', 400);}
...
router.put('/:id', parseBody(adminUpdateServerBody), async (req, res) => {
  ...
  if (!existing) {return jsonError(res, 'NOT_FOUND', 'Server not found', 404);}
