...
router.get('/', async (req, res) => {
  ...
  if (!resolved) {return;}
  if (!requireSubUserPermission(res, resolved, 'schedule.read')) {return;}
...
router.post('/', parseBody(createScheduleBody), async (req, res) => {
  ...
  if (!requireSubUserPermission(res, resolved, 'schedule.create')) {return;}
...
router.patch(
  ...
  async (req, res) => {
    ...
    if (data.timeOffset !== undefined) {updateData.timeOffset = data.timeOffset;}
...
router.delete('/:scheduleId', async (req, res) => {
  const resolved = await resolveServer(req, res);
  if (!resolved) {return;}
  if (!requireSubUserPermission(res, resolved, 'schedule.delete')) {return;}
