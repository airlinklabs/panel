  ...
  router: () => {
    ...
    router.post(
      ...
      async (req: Request, res: Response) => {
        ...
        const cleanDesc = normalizeUserText(
          description,
      ...
      async (req: Request, res: Response) => {
        ...
        if (
          ...
          res
            .status(400)
            .send(
              `Username must be ${USERNAME_MIN_LENGTH}–${USERNAME_MAX_LENGTH} characters and contain only letters, numbers, underscores, or hyphens.`,
            );
          return;
        }
