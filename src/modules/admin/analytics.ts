    ...
    router.get(
      ...
      async (_req: Request, res: Response) => {
        try {
          ...
          // — Servers section
          const totalAllocatedRam = servers.reduce(
            (s, srv) => s + (srv.Memory || 0),
            0,
          );
          const totalAllocatedCpu = servers.reduce(
            (s, srv) => s + (srv.Cpu || 0),
            0,
          );
          const totalAllocatedStorage = servers.reduce(
            (s, srv) => s + (srv.Storage || 0),
            0,
          );

          const imageCounts: Record<
            string,
            { name: string | null; count: number }
          > = {};
          images.forEach((img) => {
            imageCounts[img.id] = { name: img.name, count: 0 };
          });
          servers.forEach((srv) => {
            if (imageCounts[srv.imageId]) {
              imageCounts[srv.imageId]!.count++;
            }
          });
            ...
            nodes.map(async (node) => {
              ...
              try {
                ...
                return {
                  ...
                  disk: node.disk,
                  versionFamily:
                    (r.data as Record<string, unknown>)?.versionFamily ?? null,
                  versionRelease:
                    (r.data as Record<string, unknown>)?.versionRelease ?? null,
                };
              } catch {
                return {
                  id: node.id,
          ...
          const recentLogins = loginHistory.filter(
            (l) => new Date(l.timestamp) >= last30Days,
          );
