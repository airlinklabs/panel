...
const loader = readFileSync(resolve(__dirname, '../public/javascript/shared/page-loader.js'), 'utf8');
const htmx = readFileSync(resolve(__dirname, '../public/javascript/shared/htmx-bootstrap.js'), 'utf8');
...
describe('shared page loading feedback', () => {
  ...
  it('keeps HTMX requests out of hard-navigation handling and gives them loader feedback', () => {
    expect(loader).toContain("form.matches('[hx-get], [hx-post], [hx-put], [hx-patch], [hx-delete]')");
  ...
  it('keeps an invalid HTMX CSRF token in place instead of forcing a logout', () => {
    const csrf = readFileSync(resolve(__dirname, '../src/handlers/utils/security/csrfProtection.ts'), 'utf8');
    expect(csrf).toContain("'HX-Trigger'");
    expect(csrf).toContain('Your security token expired. Refresh the page and try again.');
