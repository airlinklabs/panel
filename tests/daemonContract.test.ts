...
describe('Cross-Repo Contract: HMAC Signature Format', () => {
  ...
  it('produces 64-char hex signature', () => {
    const sig = hmacSign(TEST_KEY, 'GET', '/api/test', '', 1234567890, 'abc123');
    expect(sig).toMatch(/^[0-9a-f]{64}$/);
  });

  it('includes timestamp, nonce, method, path, bodyRepr in payload', () => {
    // Same inputs produce same signature
    const sig1 = hmacSign(TEST_KEY, 'POST', '/api/test', 'digest:abc', 1000, 'nonce1');
    const sig2 = hmacSign(TEST_KEY, 'POST', '/api/test', 'digest:abc', 1000, 'nonce1');
...
describe('Cross-Repo Contract: Panel HMAC Signing', () => {
  ...
  beforeAll(() => {
    panelDaemonRequestSrc = fs.readFileSync(
      path.join(PANEL_ROOT, 'src', 'handlers', 'utils', 'core', 'daemonRequest.ts'),
      'utf8',
    );
  });
...
describe('Cross-Repo Contract: Signature Byte-Level', () => {
  ...
  it('panel and daemon produce identical signatures for same inputs', () => {
    ...
    const panelSig = createHmac('sha256', KEY).update(panelPayload).digest('hex');

    // Daemon's signing logic (from hmac.ts sign function)
    const daemonPayload = `${TIMESTAMP}:${NONCE}:${METHOD.toUpperCase()}:${PATH}:${BODY_REPR}`;
    const daemonSig = createHmac('sha256', KEY).update(daemonPayload).digest('hex');
  ...
  it('signature changes when any component changes', () => {
    ...
    // Change nonce
    const sig2 = hmacSign(KEY, METHOD, PATH, BODY_REPR, TIMESTAMP, 'different-nonce');
    ...
    // Change path
    const sig4 = hmacSign(KEY, METHOD, '/other/path', BODY_REPR, TIMESTAMP, NONCE);
